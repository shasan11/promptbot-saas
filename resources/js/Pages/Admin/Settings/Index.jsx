...
function FieldControl({ field, value, onChange }) {
    ...
    if (field.type === 'textarea') {
        return <textarea className={`${inputClass} mt-2 min-h-28`} value={value ?? ''} placeholder={field.placeholder || ''} onChange={(event) => onChange(event.target.value)} />;
    }
...
export default function Index({ groups = [] }) {
    ...
    return (
        <AuthenticatedLayout header={<PageHeader title="General Settings" subtitle="Configure platform identity, email, mail delivery, payments, AI/RAG, and branding." />}>
